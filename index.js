...
function addArtist(array, strName, strYears, strGenre, strNationality, strBio) {
  array.push({name: strName, years: strYears, genre: strGenre, nationality: strNationality, bio: strBio});
  return array;
}

addArtist(artists, "Aodhan", 20, "Rock", "Irish", "GOAT");
...
function artistByCountry(array, nationality){
  let filteredArray = [];
  for(let i = 0; i < array.length; i++){
    if(nationality === array[i].nationality){
      filteredArray.push(array[i].name);
    }
  }
  return filteredArray;
